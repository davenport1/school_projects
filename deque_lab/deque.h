...
namespace coen79_lab8
{
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            *first_bp = new value_type[block_size];         //create new block of block_size, first entry is new entry
            *first_bp[0] = entry;
        ...
        {
            *(--front_ptr) = entry;                         //decrement pointer and assign value
        }
        ...
        {
            --first_bp;                                             //pointer arithmetic to NULL ptr above prev first_bp
            *first_bp = new value_type[block_size];                 //create a new block of data

            front_ptr = (*first_bp) + (block_size-1);               //assign front_ptr to last element in new block
            *front_ptr = entry;                                     //assigns entry to front_ptr
        }
        ...
        {
            reserve();                                              //expand deque to accomadate more block pointers
            --first_bp;                                             //same method as previous else-if
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            *first_bp = new value_type[block_size];     //create new block of block_size, first entry is new entry
            *first_bp[0] = entry;
            back_ptr = front_ptr = *first_bp;           //front and back ptr is first element in new block
        ...
        {
            *(++back_ptr) = entry;                      //increment back_ptr and assign entry
        }
        ...
        {
            ++last_bp;                                  //first move to next NULL ptr in block pointers array
            *last_bp = new value_type[block_size];      //create new block
            back_ptr = *last_bp;                        //back_ptr will be first element in new block
            *back_ptr = entry;                          //assign entry to back ptr
        }
        ...
        {
            reserve();                                  //reserve space for more block ptrs and use same method as above
            ++last_bp;
