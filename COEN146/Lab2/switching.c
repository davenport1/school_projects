...
// # Date: 9/28/2022
// # Title: Lab2 – packetswitching vs circuit switching
// # Description: This program demonstrates difference in packetswitching 
//      and circuit switching efficiencys
#include <stdio.h>  
#include <math.h>
#include <stdlib.h>
...
int main(int argc, char *argv[]) {
   ...
   // 2a: Circuit Switching Senario
    ...
    double nPSusersChooseTen = factorial(nPSusers) / (factorial(10) * factorial(nPSusers - 10));
    double pAnyTenBusy = nPSusersChooseTen * pow(pPSusers, 10) * pow(pPSusersNotBusy, nPSusers - 10);
    double pTenMoreBusy = 0;
    for(i = 11; i < nPSusers; i++) {
        double nPSuserChooseI = factorial(nPSusers) / (factorial(i) * factorial(nPSusers - i));
        pTenMoreBusy+= nPSuserChooseI * pow(pPSusers, i) * pow(pPSusersNotBusy, nPSusers - i);
    }
