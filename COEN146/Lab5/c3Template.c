...
void clientSend(int sockfd, const struct sockaddr *address, socklen_t addrlen, Packet packet, unsigned retries) {
    while (1) {	
        ...
        //so, there is an ACK to receive
            ...
            int rp = recvfrom(sockfd, &recvpacket, sizeof(recvpacket), 0, (struct sockaddr *)address, &addrlen); 
            ...
            int rv_cksum = getChecksum(recvpacket);
            //check the checksum
            if(recvpacket.header.cksum != rv_cksum) {
